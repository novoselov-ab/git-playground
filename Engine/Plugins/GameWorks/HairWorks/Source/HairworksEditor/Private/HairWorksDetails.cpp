...
void FHairWorksMaterialDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	auto IsEditable = [](TSharedRef<IPropertyHandle> PropertyHandle, TSharedRef<TArray<TWeakObjectPtr<UHairWorksMaterial>>> SelectedObjects)
	{
		if(!PropertyHandle->IsValidHandle())
			return false;
	...
	// To Bind handles
	auto AddPropertyHandler = [&](IDetailPropertyRow& DetailProperty, const FName& PropertyName)
	{
	...
	for(auto& Category : TopCategory.Categories)
	{
		...
		for(auto& PropertyName : Category->Properties)
		{
			...
			AddPropertyHandler(DetailProperty, PropertyName);
		}
