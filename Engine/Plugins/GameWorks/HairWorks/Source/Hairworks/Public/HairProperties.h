...
#include "HairProperties.generated.h"

struct GFSDK_HairInstanceDescriptor;

/**
* HairProperty represents property of a hair.
*/
USTRUCT()
struct HAIRWORKS_API FHairProperties
{
...
public:
	...
	// Takes values from the provided descriptor and plugs it into this structure.
	void ImportPropertiesFrom(const GFSDK_HairInstanceDescriptor& Descriptor);

	// Plugs the values from this structure into the provided descriptor
	void ExportPropertiesTo(GFSDK_HairInstanceDescriptor& Descriptor);

	const TArray<FTexture2DRHIRef>& GetTextures();
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Physical|General", meta = (ClampMin = "0", ClampMax = "100"))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Style|StrandWidth", meta = (DisplayName = "Scale", ClampMin = "0", ClampMax = "10"))
	float WidthScale = 1;

	// Control map for hair width.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Style|StrandWidth", meta = (DisplayName = "Scale Map"))
	UTexture2D* WidthScaleMap;

	// Scale factor for top side of the strand.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Style|StrandWidth", meta = (DisplayName = "Root", ClampMin = "0", ClampMax = "1"))
	float WidthRootScale = 1;

	// Scale factor for bottom side of the strand.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Style|StrandWidth", meta = (DisplayName = "Tip", ClampMin = "0", ClampMax = "1"))
	...
	// When this is on, density for hairs growing from backfacing faces will be set to 0.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "LevelOfDetail|Culling")
	bool bBackfaceCulling = false;

	// Threshold to determine backface, note that this value should be slightly smaller 0 to avoid hairs at the silhouette from disappearing.
...
private:
	mutable TArray<FTexture2DRHIRef> CachedTextures;
