...
void UHairComponent::CreateRenderState_Concurrent()
{
	...
	if (SceneProxy)
	{
		...
		if (Hair)
			GHairManager->GetHairInfo_GameThread(DefaultHairDesc, HairBoneToIdxMap, *Hair);
...
void UHairComponent::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
		...
		)
	{
		...
		if (Hair)
		{
			if (GHairManager->GetHairInfo_GameThread(DefaultHairDesc, HairBoneToIdxMap, *Hair))	// Proxy may have not been created yet. So we get information from asset.
			{
				TArray<FTexture2DRHIRef> HairTextures;
